...
from gaussian_renderer import render_fastgs as render
...
def evaluate_set(name, views, gaussians, pipeline, background, args):
    """
    ...
    with torch.no_grad():
        ...
        if not skip_train:
            ...
            train_mean, train_per_view = evaluate_set(
                "train", train_views, gaussians, pipeline, background, args
            )
        ...
        if not skip_test:
            ...
            test_mean, test_per_view = evaluate_set(
                "test", test_views, gaussians, pipeline, background, args
            )
